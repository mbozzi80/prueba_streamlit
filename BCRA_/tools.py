...
def addSignal(data: pd.DataFrame, 
              fast: int = 5, # La cantidad de valores para la media móvil rápida
              slow: int = 20,  # La cantidad de valores para la media móvil lenta
    ...
    # Calcular el cruce de medias
    data['Cruce'] = (data.close.rolling(fast).mean() / data.close.rolling(slow).mean() - 1) * 100

    # Calcular el RSI
    dif = data['close'].diff()
    win = pd.Series(np.where(dif > 0, dif, 0), index=data.index)
    loss = pd.Series(np.where(dif < 0, abs(dif), 0), index=data.index)
    ema_win = win.ewm(alpha=1 / rsi_q, adjust=False).mean()
    ema_loss = loss.ewm(alpha=1 / rsi_q, adjust=False).mean()
    rs = ema_win / ema_loss
    rsi = 100 - (100 / (1 + rs))
    data['rsi'] = rsi

    # Inicializar la columna 'signal' con un valor por defecto
    ...
    # Asignar señales iniciales de compra y venta
